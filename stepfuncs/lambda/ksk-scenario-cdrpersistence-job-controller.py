...
SSM_PARAMETER_JOB_CDRPERSISTENCE_DESIRED_PROCESS_COUNT='ksk-scenario-cdrpersistence-desired-process-count'

##  stepfunction
#stm_cdrpersistence_arn='arn:aws:states:ap-northeast-2:091262214952:stateMachine:ksk-scenario-stm-cdrpersistence'
stm_cdrpersistence_arn='arn:aws:states:ap-northeast-2:091262214952:stateMachine:ksk-scenario-stm-cdrpersistence-choice'
...
def get_running_stepfunction_count(desired_count):
    try:
        response = stepfunc.list_executions(
                stateMachineArn=stm_cdrpersistence_arn,
...
def execute_rating_stepfunction(more_runnable_count, awsbatchtype):
    while more_runnable_count > 0 :
        response = receive_sqs_message(cdrpersistenceQURL, more_runnable_count, Wait_Time)
        ...
        if response['ResponseMetadata']['HTTPStatusCode'] == 200 :
            if 'Messages' not in response :
                ...
                for msg in response['Messages']:
                    ...
                    delete_sqs_message(cdrpersistenceQURL, msg['ReceiptHandle'])
...
def start_rating_stepfunction(inputfilename, awsbatchtype):
    try:
        response = stepfunc.start_execution(
                              stateMachineArn=stm_cdrpersistence_arn,
                              input=json.dumps({"InputFileName": inputfilename, "Caller": "ksk-scenario-cdrpersistence-job-controller", "RunEnvironment": awsbatchtype})
                   )
    except Exception as e:
        print(e)
...
def lambda_handler(event, context):
    ...
    while loopcnt <= 280 : 
        ...
        if holding_yn == 'N' :  # 정상 상태
            ...
            # get cdrpersistence process desired count info
            desired_count = get_parameter(SSM_PARAMETER_JOB_CDRPERSISTENCE_DESIRED_PROCESS_COUNT)
